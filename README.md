...
### 首次运行提示“未知发布者/Windows 保护了你的电脑”

这是 Windows 的 SmartScreen/安全策略提示，常见于未签名的可执行文件。仅靠代码无法彻底消除；最佳实践是对发布的 `.exe` 进行代码签名（Code Signing）。

临时处理方式（自行评估风险）：
- 右键文件 → 属性 → 勾选“解除锁定/Unblock”（如果有）
- 或用 PowerShell：`Unblock-File .\\autoloot-v*.exe`

### 为什么需要管理员权限运行

当目标程序以管理员权限运行时，普通权限的脚本/程序通常无法向其发送输入或进行窗口操作。为保证兼容性，本项目会在启动时自动请求提升权限（会弹出 UAC 提示）。
